...
class LOGTransformation:
    """
    The LOG Transformation transforms logical operations mainly between booleans. Here the type of the
    ...
    """
    TYPE = "LOG"
    OPERATORS = ["and", "not and", "and not", "or", "or not", "not or", "not or not", "not and not"]
    currentHoles = []
    ...
    def clear_current_holes():
        ...
        """
        LOGTransformation.currentHoles = []
    ...
    def add_to_current_holes(hole_new):
        ...
        """
        for hole in LOGTransformation.currentHoles:
            if hole.is_equal(hole_new):
                raise RuntimeError(
                    "The hole:" + str(hole_new.to_array()) + " could not be inserted because it is already in the "
                    + LOGTransformation.TYPE + " Transformation.")
        LOGTransformation.currentHoles.append(hole_new)
    ...
    def call(hole_number: int, line_number: int, parent: bool, left_value_array, right_value_array):
        ...
        transformations or whole code statements e.g. (a and b). Then a random operator is chosen ad the changed code
        and value is calculated.
        ...
        # taken again.
        current_hole = LOGTransformation.find_hole(hole_number)
        if current_hole is not None:
            ...
            randomNumber = random.randrange(0, len(LOGTransformation.OPERATORS), 1)
            operator = LOGTransformation.OPERATORS[randomNumber]
        ...
        # are the same operation later.
        value = None
        changed_code_array = []
        if operator == "or":
            value = left_operand_value or right_operand_value
            changed_code_array = [left_operand + " or " + right_operand,
                                  right_operand + " or " + left_operand]

        elif operator == "and":
            value = left_operand_value and right_operand_value
            changed_code_array = [left_operand + " and " + right_operand,
                                  right_operand + " and " + left_operand]

        elif operator == "or not":
            value = left_operand_value or not right_operand_value
            changed_code_array = [left_operand + " or not " + right_operand,
                                  "not " + right_operand + " or " + left_operand]

        elif operator == "not or":
            value = not left_operand_value or right_operand_value
            changed_code_array = ["not " + left_operand + " or " + right_operand,
                                  right_operand + " or not " + left_operand]

        elif operator == "and not":
            value = left_operand_value and not right_operand_value
            changed_code_array = [left_operand + " and not " + right_operand,
                                  "not " + right_operand + " and " + left_operand]

        elif operator == "not and":
            value = not left_operand_value and right_operand_value
            changed_code_array = ["not " + left_operand + " and " + right_operand,
                                  right_operand + " and not " + left_operand]

        elif operator == "not or not":
            # Here also rule of de Morgan is taken to ensure no duplicates.
            value = not left_operand_value or not right_operand_value
            changed_code_array = ["not " + left_operand + " or not" + right_operand,
                                  "not " + right_operand + " or not " + left_operand,
                                  "not (" + left_operand + " and " + right_operand + ")",
                                  "not (" + right_operand + " and " + left_operand + ")"]

        elif operator == "not and not":
            # Here also rule of de Morgan is taken to ensure no duplicates.
            value = not left_operand_value and not right_operand_value
            changed_code_array = ["not " + left_operand + " and not" + right_operand,
                                  "not " + right_operand + " and not " + left_operand,
                                  "not (" + left_operand + " or " + right_operand + ")",
                                  "not (" + right_operand + " or " + left_operand + ")"]
        ...
        if current_hole is None:
            current_hole = Hole(hole_number, line_number, changed_code_array, LOGTransformation.TYPE, operator)
            if not (left_operand == right_operand or (left_operand.isdigit() or right_operand.isdigit())):
                LOGTransformation.add_to_current_holes(current_hole)
                if not parent:
